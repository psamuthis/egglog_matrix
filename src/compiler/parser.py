...
MATRIX_RAW = rf"Matrix\(\"\w*\", \d+,\s*\d+,\s*\d*\.\d+\)"
MATRIX_TOKEN = rf"Matrix\(\"\w*\", \d+,\s*\d+,\s*\d*\.\d+\)(?:\.{UnaryMatrixOp.ANY()}\(\))*"
...
def build_compute_graph(expr: str, debug: str="", depth: int=0) -> ExpressionTree:
    ...
    assert balance == 0, f"build_compute_graph: input expr not balanced. {expr}"
