...
@egraph.register
def _matrix_multiplication(x: Matrix, y: Matrix, z: Matrix, id: String, r: i64, c: i64, m: i64, s: f64) -> Iterable[RewriteOrRule]:
    yield rule(x == Matrix(id, r, c, s)).then(set_(x.id).to(id), set_(x.row).to(r), set_(x.col).to(c), set_(x.sparsity).to(s))
    yield rule(
        x == (y @ z),
        y.col == z.row,
